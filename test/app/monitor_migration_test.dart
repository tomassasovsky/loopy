...
void main() {
  group('runMonitorMigration', () {
    ...
    group('v1 (global flag → per-input), then v2 folds it into lanes', () {
      ...
      test('a fresh install (no legacy flag) just marks both migrations done',
          () async {
        await runMonitorMigration(settings);
    ...
    group('v2 (single route → lanes)', () {
      ...
      test('a wet + dry legacy route becomes lane 0 (wet) + lane 1 (clean)',
          () async {
      ...
      test('a second run is a no-op once the v2 flag is set (idempotent)',
          () async {
        await settings.saveMonitorInput(0, enabled: true, outputMask: 0x1);
        await runMonitorMigration(settings);
      ...
      test('a store on v1 but not v2 still converts on the next launch',
          () async {
