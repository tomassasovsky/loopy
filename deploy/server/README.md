...
Pi polls (and desktop Sparkle/WinSparkle appcasts later). Runs behind the user's
nginx, which reverse-proxies `https://segno.aquiles.dev` → `127.0.0.1:3029`.
...
## Deploy on Portainer
1. Portainer → Stacks → Add stack → paste `docker-compose.yml` (this dir).
2. It needs the two files next to the compose on the host: `default.conf` and the
   `www/` tree. Either clone this `deploy/server/` dir onto the Portainer host and
   point the stack at it, or use a Portainer "git repository" stack targeting this
   path in the repo.
3. Deploy. It publishes host port **3029**; your existing nginx vhost for
   `segno.aquiles.dev` already proxies to it.
