#!/usr/bin/env bash
# Tests for how `loopy-wifi-ctl connect` explains a failed join (#466).
#
...
run_connect() {
    LOOPY_WIFI_IFACE=wlan0 \
    LOOPY_JOURNALCTL="$work/bin/journalctl" \
    LOOPY_WIFI_NM_CONNECT_TIMEOUT=1 \
    LOOPY_WIFI_CONNECT_TIMEOUT=1 \
    PATH="$work/bin:$PATH" \
