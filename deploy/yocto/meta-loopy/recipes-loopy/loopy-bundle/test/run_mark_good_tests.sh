...
SCRIPT="$here/../files/loopy-mark-good"
...
run_mark_good() {
    LOOPY_SYSTEMCTL="$work/bin/systemctl" \
    LOOPY_RAUC="$work/bin/rauc" \
    LOOPY_MARK_GOOD_WAIT_SECS="${WAIT_OVERRIDE:-4}" \
    LOOPY_MARK_GOOD_STABLE_SECS="${STABLE_OVERRIDE:-1}" \
