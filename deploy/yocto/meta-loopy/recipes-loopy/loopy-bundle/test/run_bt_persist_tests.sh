#!/usr/bin/env bash
# Tests for `loopy-bt-persist` (#451).
#
...
SCRIPT="$here/../files/loopy-bt-persist"
...
run_persist() {
    LOOPY_BT_DATA_DIR="$work/data/bluetooth" \
    LOOPY_BT_STATE_DIR="$work/state/bluetooth" \
    LOOPY_MOUNT="$work/bin/mount" \
    LOOPY_MOUNTINFO="$work/mountinfo" \
