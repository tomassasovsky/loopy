 ...
 * exclusion (see le_platform_excluded_input_mask). The frameworks are already
...
/* dlopen libjack at runtime to repin the JACK ports to the selected device
 * (le_jack_pin_to_device); libjack is already loaded by miniaudio's backend. */
#include <dlfcn.h>
/* Forward-declared: used by le_engine_destroy, which precedes its definition. */
...
/* Seam: excluded-input bitmask for the capture device identified by `uid`
 * (NULL/empty => default input). macOS reads Core Audio channel labels; every
 * other platform excludes nothing. (Step 2: body still here, guarded; moves to
 * engine_apple.c in step 3.) */
 ...
 *
 * Step 2 of the per-OS subdivision: the five le_platform_* hooks are defined
 * here, each still wrapping the platform's existing `#if` block verbatim. Steps
 * 3-5 move the active bodies out to engine_apple.c / engine_linux.c /
 * engine_windows.c; the portable call sites above already go through the seam.
 * (le_platform_excluded_input_mask sits beside its Apple body, above.) */
...
void le_platform_backends(const ma_backend** out_list, ma_uint32* out_count) {
  ...
  *out_count = 3;
#else
  /* Every other platform keeps miniaudio's default backend priority. */
  *out_list = NULL;
  *out_count = 0;
#endif
}
