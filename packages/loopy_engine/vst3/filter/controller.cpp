...
tresult PLUGIN_API Controller::initialize(FUnknown* context) {
  ...
  le_fx_defaults(LE_FX_FILTER, defaults);
  ...
  parameters.addParameter(
      new RangeParameter(STR16("Cutoff"), kCutoffId, nullptr, 0., 1., defaults[0]));
  parameters.addParameter(new RangeParameter(STR16("Resonance"), kResonanceId, nullptr,
                                             0., 1., defaults[1]));
...
tresult PLUGIN_API Controller::setComponentState(IBStream* state) {
  ...
  setParamNormalized(kCutoffId, saved[0]);
  setParamNormalized(kResonanceId, saved[1]);
  return kResultOk;
}
