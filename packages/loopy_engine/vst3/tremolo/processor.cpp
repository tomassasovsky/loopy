...
tresult PLUGIN_API Processor::initialize(FUnknown* context) {
  ...
  types_[0] = LE_FX_TREMOLO;
  le_fx_defaults(LE_FX_TREMOLO, params_[0]);
  // LE_FX_TREMOLO's vtable row has a NULL `prepare` (engine_fx.c) — no heap
  ...
  // effect type.
  if (le_fx_prepare(&fx_, 0, LE_FX_TREMOLO, /*cap=*/0) != LE_OK) {
    return kResultFalse;
  }
  return kResultOk;
}
...
tresult PLUGIN_API Processor::terminate() {
  // LE_FX_TREMOLO's vtable `prepare` is NULL (engine_fx.c) — this slot
  // never allocates a delay ring or octaver buffers, so there is nothing to
  // free here. Its sine-LFO phase lives inline in le_fx_state and is
  // cleared by le_fx_entry_reset, not allocated/freed.
...
tresult PLUGIN_API Processor::process(ProcessData& data) {
  ...
  if (IParameterChanges* changes = data.inputParameterChanges) {
    ...
    for (int32 i = 0; i < count; ++i) {
      ...
      switch (queue->getParameterId()) {
        case kRateId: params_[0][0] = static_cast<float>(value); break;
        case kDepthId: params_[0][1] = static_cast<float>(value); break;
        default: break;
      }
    }
  }
  ...
  // time. fx_tremolo tracks each channel's LFO phase independently
  // (fx->lfo[slot][chan]) with no cross-channel coupling, so aliasing
  // channel 1 onto channel 0 here reproduces the engine's own
  // mono-seeds-l-equals-r convention exactly (engine_private.h) — both
  // channels' LFOs start in phase and stay there since they receive
  // identical input.
