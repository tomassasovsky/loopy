...
tresult PLUGIN_API Processor::initialize(FUnknown* context) {
  ...
  types_[0] = LE_FX_DELAY;
  le_fx_defaults(LE_FX_DELAY, params_[0]);
  if (le_fx_prepare(&fx_, 0, LE_FX_DELAY, kDelayCapFrames) != LE_OK) {
    return kResultFalse;
  }
  return kResultOk;
}
...
tresult PLUGIN_API Processor::process(ProcessData& data) {
  ...
  for (int32 i = 0; i < data.numSamples; ++i) {
    float l = inL[i];
    float r = inR[i];
    fx_apply_chain(&fx_, sr, kDelayCapFrames, &l, &r, 1, types_, params_);
    outL[i] = l;
