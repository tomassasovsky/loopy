# Loopy foot-pedal â€” PCB design (standalone, 2 boards)
...
> the 9 Vâ†’5 V **buck**, the **footswitch inputs** (with hardware RC debounce), the
...
```
  9V DC â”€â–¶ rev-prot â”€â–¶ BUCK 5V(3A) â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¶ 5V_LED (both LED strips)
                                     â””â”€â”€[ideal-diode OR]â”€â”€â”
  USB-C 5V â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€[ideal-diode OR]â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â–¶ 5V_LOGIC (328P + 16U2)

  MIDI  | USB-C D+/D- â–¶ 16U2 (dualMoco USB-MIDI)  TXD â”€â”
        |                                              â”œâ”€[74HC08 AND merge]â”€â–¶ 328P RXD
        | DIN MIDI IN â–¶ H11L1 opto â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
        | 328P TXD â”€â”¬â”€â–¶ 16U2 RXD (USB out)
        |           â””â”€â–¶ 74AHCT125 buf â”€â–¶ DIN MIDI OUT
        | (same 31250 UART â†’ USB and DIN carry the identical pedal protocol)

  I/O   | 328P â”€ D3..D12 â”€â”€â–¶ 10Ã— footswitches (RC debounce, connectors)
        |      â”€ D2 â”€â”€â”€â”€â”€â”€â”€â–¶ indicator strip: 7Ã— WS2812 (main board, idx 0..6)
        |      â”€ A3 â”€â”€â”
        |      â”€ A0/A1/A2 (encoder) â”€â”€â”
        |              â””â”€ 8-pin cable â”€â–¶ RING BOARD: 12Ã— WS2812 ring (A3 data) + EC11
```

- **Logic (328P + 16U2)** runs from `5V_LOGIC` = OR(USB 5 V, buck 5 V) â†’ works on
...
- **MIDI** is available over **USB and DIN at once** (output) / either source
  (input, Â§6), so the pedal works plugged into a computer *or* an interface's MIDI.
- Single common ground (the DIN **IN** is opto-isolated to break ground loops).

---

## 2. LED outputs â€” two independent WS2812 strips
...
- **Indicator strip** â€” **D2 (PD2)**, **7 LEDs on the main board**, indices 0â€“6:
...
- **Ring strip** â€” **A3 (PC3)**, **12 LEDs on the ring board**, indices 0â€“11, fed
  over the cable.

Each strip is its own chain indexed from 0 â€” **no cross-board chaining, no data
...
> from 0. (No chain re-ordering needed â€” simpler than the single-chain plan.)

Data integrity: **330 Î©** series at each data pin; a **74AHCT125** buffer on the
**ring** data line **before the cable** (clean 5 V over the ribbon); **1000 ÂµF**
...
often center-negative â€” pick one and silkscreen it). Add a **TVS** (SMBJ12A) and a
**reverse-polarity P-MOSFET** (e.g., DMP3098L: sourceâ†’jack+, drainâ†’VIN, gateâ†’GND)
across the input.

**Buck (9 Vâ†’5 V):** **MP1584EN** (SOT-23-8) or **MP2315** (3 A) switching regulator,
set to 5.0 V. Reference MP1584 application circuit: 10 ÂµH inductor, SS36 catch diode
(if not synchronous), 22 ÂµF in / 2Ã— 22 ÂµF out, FB divider for 5.0 V. Budget below
...
| rail | load | current |
|------|------|---------|
| 5V_LED | 19 Ã— WS2812B @ 60 mA | 1.14 A |
| 5V_LOGIC | 328P + 16U2 + USB | ~0.07 A |
| **total @ 5 V** | | **~1.2 A** (â†’ ~0.75 A from 9 V through the buck @ ~88 %) |

Real-world (a few LEDs lit, breathing) is far lower; size for the 1.2 A worst case.
Barrel jack / buck inductor / cable conductors rated â‰¥ 2 A.

---

## 4. MCU subsystem â€” ATmega328P-AU (TQFP-32)

Lifted from the UNO R3 "main MCU" block:

- **Clock:** 16 MHz crystal on XTAL1/XTAL2 (pins 7/8) + 2Ã— 22 pF to GND.
- **Reset:** 10 kÎ© from RESET (pin 29) to 5V_LOGIC; reset tact switch to GND;
  **100 nF** from the 16U2's DTR line to RESET (auto-reset on sketch upload).
- **Decoupling:** 100 nF on each VCC (pins 4, 6); 100 nF + 10 ÂµF bulk; **AVCC**
  (pin 18) via a 10 ÂµH ferrite/inductor from 5 V + 100 nF; AREF 100 nF.
- **ICSP-328P:** 2Ã—3 header (MISO/MOSI/SCK/RESET/VCC/GND) for bootloader burning.

### 328P pin assignment (matches the firmware)

| Port / pin | Arduino | Net | Use |
|------------|---------|-----|-----|
| PD0 (RXD) | D0 | UART_RX â† 16U2 TXD | serial MIDI in (31250) |
| PD1 (TXD) | D1 | UART_TX â†’ 16U2 RXD | serial MIDI out (31250) |
| **PD2** | **D2** | **IND_DATA** | indicator-LED strip (7), main board, 330 Î© series |
...
| **PC3** | **A3** | **RING_DATA** | ring-LED strip (12) â†’ 74AHCT125 â†’ cable |
| PB5 | D13 | (ICSP SCK) | â€” (free; onboard "alive" LED optional) |
...
## 5. USB-MIDI subsystem â€” ATmega16U2-AU (TQFP-32)

Verbatim from the UNO R3 "USB interface" block:

- **USB-C** receptacle (USB 2.0): VBUSâ†’USB 5 V, D+/Dâˆ’ through **22 Î©** series each to
  the 16U2 D+/Dâˆ’ (pins 4/3 on the 16U2 are the differential pair per the UNO ref;
  use the symbol's UDP/UDM), **CC1/CC2 â†’ 5.1 kÎ©** to GND (advertise UFP/device).
- **Clock:** 16 MHz crystal + 2Ã— 22 pF.
- **16U2 â†” 328P (shared UART):** 16U2 PD3(RXD) â† 328P TXD (output to USB **and** the
  DIN MIDI OUT in parallel); 328P RXD â† the **MIDI-IN merge** (Â§6), one input of
  which is 16U2 PD2(TXD). 16U2 PD7 â†’ the 100 nF â†’ 328P RESET (DTR auto-reset). Test
  pads/jumper for the dualMoco **MOSI/PB2 â†’ GND** "serial mode" select.
- **ESD:** USBLC6-2SC6 on D+/Dâˆ’/VBUS.
- **ICSP-16U2:** 2Ã—3 header (for DFU recovery / dualMoco flashing).
- **RESET-16U2:** tact switch + 10 kÎ© pull-up (the DFU-entry RESET+GND we used).

---

## 6. Hardware MIDI (5-pin DIN) â€” standalone / interface mode
...
bridges to USB â€” so USB-MIDI and DIN-MIDI carry the identical pedal protocol, and
**no firmware change** is needed. In this mode loopy simply selects the
*interface's* MIDI in/out instead of "MocoLUFA".

```
328P TXD â”€â”€â”¬â”€â”€â–¶ 16U2 RXD            (USB-MIDI out)
           â””â”€â”€â–¶ 74AHCT125 buf â”€220Î©â”€â–¶ DIN-OUT pin 5   (+5Vâ”€220Î©â”€â–¶ pin 4, pin 2â”€GND)

DIN-IN â”€â–¶ H11L1 opto â”€â”
                      â”œâ”€[74HC08 AND]â”€â”€â–¶ 328P RXD       (MIDI-IN merge)
16U2 TXD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                                (USB-MIDI in)
```

- **MIDI OUT** (not isolated, per spec): 328P TXD â†’ one spare **74AHCT125** buffer â†’
  220 Î© â†’ DIN pin 5; +5 V â†’ 220 Î© â†’ DIN pin 4; pin 2 â†’ GND. Drives USB and DIN at
  once (harmless when one side is unused).
- **MIDI IN** (isolated): DIN pin 4 â†’ 220 Î© â†’ **H11L1** opto input; pin 5 â†’ opto
...
- **MIDI-IN merge:** a single **74HC08** AND gate combines `16U2 TXD` and the opto
  output into `328P RXD`. Both idle HIGH; whichever source transmits pulls RXD â€”
...
- **Connectors:** 2Ã— panel **DIN-5** (180Â°) jacks, or 2Ã— **3.5 mm TRS (MIDI Type-A)**
  for a compact build â€” wire pins 4/5/2 to TRS tip/ring/sleeve per Type-A.
...
- **100 nF** across the switch (pinâ†’GND) for hardware RC debounce (~few ms with the
  internal pull-up) â€” belt-and-suspenders with the firmware's 25 ms debounce.
- Optional **100 Î©** series + **TVS/ESD** if the switch wiring is long.
...
- **12Ã— WS2812B** in a circle (loop ring), chain indices 7â€“18, DIN from the cable,
  DOUT unused (end of chain). 100 nF per ~4 LEDs + **1000 ÂµF** bulk across 5 V at DIN.
- **EC11 rotary encoder** (with push switch): Aâ†’ENC_A, Bâ†’ENC_B, C(common)â†’GND,
  switchâ†’ENC_SW + GND. Add **10 kÎ© pull-ups + 100 nF** RC on A/B (encoders bounce).
- **Inter-board connector** (8-pin JST-XH, see Â§9).
- Center cutout / mount for the encoder shaft; ring LED pitch sized to the enclosure.

---

## 9. Inter-board connector (main â†” ring), 8-pin JST-XH
...
Keep the cable â‰¤ ~30 cm; the 74AHCT125 buffer + doubled power pins handle the run.
...
| J1 | USB-C receptacle (2.0) | â€” | 1 |
| J2 | 2.1 mm barrel jack | â€” | 1 |
| J3,J4 | footswitch terminals (6-pos) | JST-XH/screw | 2 |
| J5 | ring-board header (8-pin) | JST-XH | 1 |
| J6,J7 | ICSP (2Ã—3) | 2.54 hdr | 2 |
| J8,J9 | MIDI IN / OUT (DIN-5 180Â°, or 3.5 mm TRS-A) | â€” | 2 |
| Q1 | DMP3098L reverse-prot P-FET | SOT-23 | 1 |
| L1 | 10 ÂµH â‰¥2 A buck inductor | â€” | 1 |
| D1 | SS36 (if non-sync buck) | SMA | 1 |
| D2 | 1N4148 (MIDI-IN protection) | SOD-123 | 1 |
| â€” | R/C: 22 pFÃ—4, 100 nFÃ—~13, 10 ÂµF, 22 ÂµFÃ—3, 1000 ÂµF, 10 kÎ©Ã—~4, 22 Î©Ã—2, 5.1 kÎ©Ã—2, 330 Î©, 220 Î©Ã—3 (MIDI), 100 nF debounceÃ—10 | 0603 | â€” |
| SW1,SW2 | reset tact (328P, 16U2) | â€” | 2 |
| â€” | indicator LEDs WS2812B (chain 0â€“6) | 5050 | 7 |
...
| J1 | 8-pin header | 1 |
| â€” | 1000 ÂµF, 100 nFÃ—4, 10 kÎ©Ã—2, 100 nFÃ—2 (enc RC) | â€” |

10Ã— foot switches and the barrel-jack PSU are external.
...
- **Buck:** tight input loop (Cinâ€“VINâ€“GND), keep the SW node small, inductor close,
  output cap near the IC; star/wide ground; keep the switch node away from the
  crystals and USB pair.
- **USB:** route **D+/Dâˆ’ as a ~90 Î© differential pair**, short, matched, over a solid
  ground plane; series 22 Î© near the 16U2; ESD diode at the connector.
...
- **LED power:** wide 5V_LED/GND pours (â‰¥ 1.2 A); bulk cap at each board's first LED.
- **Footswitch + encoder lines:** keep RC caps near the connector/MCU; ground-guard
  the encoder lines.
- **Mounting:** 4Ã— M3 holes per board; align the ring board's encoder + LED pitch to
...
| file | what |
|------|------|
| `kicad/main_board.py` | SKiDL source for the main board â†’ `main_board.net` (84 parts) |
| `kicad/ring_board.py` | SKiDL source for the ring board â†’ `ring_board.net` (22 parts) |
| `kicad/main_board.net`, `ring_board.net` | KiCad netlists, importable into pcbnew |

Verified here: **main board â€” 0 ERC errors** (30 warnings, all expected
unconnected pins: unused MCU GPIOs, USB-C sideband, DIN unused pins, chain-end
DOUT); **ring board â€” 0 ERC errors**. Both netlists generate with 0 errors.

KiCad-10 symbol names actually used (these differ from older libs): the MCUs are
`MCU_Microchip_ATmega:ATmega328P-A` and `â€¦:ATmega16U2-A`; the buck is
`Regulator_Switching:AP63203WU` (synchronous â€” no catch diode); the ideal diodes
are `Power_Management:LM66100DCK`; logic is single-gate `74xGxx:74AHCT1G125`
(Ã—2 buffers) and `74xGxx:74AHCT1G08` (merge); USB-C is
...
| file | what |
|------|------|
| `kicad/loopy_pedal_main.kicad_pcb` (+ `.kicad_pro`) | main board: 80Ã—66 mm, 72 parts, 2-layer, **911 track segs / 74 vias, DRC 0 errors / 0 unrouted** |
| `kicad/loopy_pedal_ring.kicad_pcb` (+ `.kicad_pro`) | ring board: âŒ€68 mm round, 22 parts, 12-LED ring + center encoder, **287 segs / 4 vias, DRC 0 / 0 unrouted** |
...
FET is **`Transistor_FET:AO3401A`** (numbered pads that match SOT-23 â€” the
generic `Q_PMOS` letter-pads do not).

**I/O layout:** all external I/O is on the **bottom edge** â€” 9 V barrel, USB-C,
and the two MIDI jacks â€” each aligned to the board boundary so the openings
overhang ~1.5â€“2 mm and stay reachable from outside the enclosure. Internal
...
(SparkFun/Tayda style) â€” footprint `loopy:MIDI_DIN5_RA` in the local
...
via OpenSCAD â€” shielded box + square flange + recessed round socket + contact
pins) is attached, oriented so the socket faces out the board edge.

**MIDI IN is opto-isolated per the MIDI spec:** DIN pin 4 â†’ 220 Î© â†’ the **H11L1
optocoupler (U9)**, DIN pin 5 â†’ cathode, a **1N4148 (D2)** across the LED for
...
**Indicator LEDs** are **not** on the main board â€” they break out through the
3-pin **J11** header (`+5V_LED` / data / `GND`) to an off-board strip, with only
the 330 Î© series resistor + decoupling staying on the main board.
...
scaled to 2.0 mm) â€” purely cosmetic, no copper/footprint change.

### How the layout was produced (scripted pipeline)

KiCad 10's GUI/standalone `pcbnew` and `kinet2pcb` both crash on netlistâ†’board
...
1. **Import** netlist â†’ footprints+nets (GUI **File â–¸ Import Netlist** once for
   the main board; a small `pcbnew` parser builds the ring board directly).
2. **Place** â€” courtyard-aware shelf-packing into functional zones, connectors
   pinned to edges, with a self-overlap check.
3. **Route** â€” export Specctra `.dsn` (`pcbnew.ExportSpecctraDSN`) â†’ **Freerouting
   1.6.5** batch (`-de -do -mp 50`) â†’ import `.ses` (`pcbnew.ImportSpecctraSES`).
   Route at 0.25 mm clearance so the optimizer's nicks stay above the 0.15 DRC limit.
4. **Pour** GND zones both layers + fill â†’ **DRC** (`kicad-cli pcb drc`) â†’ **Gerbers**
