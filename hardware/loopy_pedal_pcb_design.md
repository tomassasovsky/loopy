...
### Finish the PCB (in KiCad's pcbnew)
1. New project → open **pcbnew** → **File ▸ Import Netlist** → `main_board.net`
   (all footprints land with the ratsnest). Repeat in a second project for the ring.
...
> `10.0` install path (patchable, but the standalone-pcbnew crash remains). So the
> board is laid out by the netlist-import step above rather than auto-generated.
